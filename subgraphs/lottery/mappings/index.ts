...
export function handleLotteryOpen(event: LotteryOpen): void {
  ...
  lottery.ticketPrice = toBigDecimal(event.params.priceTicketInSafemoon);
...
export function handleTicketsPurchase(event: TicketsPurchase): void {
  ...
  if (user === null) {
    ...
    user.totalSafemoon = ZERO_BD;
    user.block = event.block.number;
    user.timestamp = event.block.timestamp;
    user.save();
  }
  user.totalTickets = user.totalTickets.plus(event.params.numberTickets);
  user.totalSafemoon = user.totalSafemoon.plus(event.params.numberTickets.toBigDecimal().times(lottery.ticketPrice));
  user.save();
...
export function handleTicketsClaim(event: TicketsClaim): void {
  ...
  if (user !== null) {
    user.totalSafemoon = user.totalSafemoon.plus(toBigDecimal(event.params.amount));
    user.save();
  }
...
lotteries[_lotteryId].safemoonPerBracket[5] = ( ( 3000 * 100TOKENS ) / 2226742 - 0 ) )/ 10000

lotteries[_lotteryId].safemoonPerBracket[j] = ((_lotteries[_lotteryId].rewardsBreakdown[j] * amountToShareToWinners) /
                            (_numberTicketsPerLotteryId[_lotteryId][transformedWinningNumber] -
                                numberAddressesInPreviousBracket)) /
                        10000;

134725980827.5947550277490611844569330438820483019586
882059786012.2959134170114050330331389861604819574671
*/